...
تم إصلاح مشكلة تكرار إرسال /start وإضافة مهلة انتظار ذكية لرد البوت.
"""
...
async def get_init_mrg(cli, bot):
    full = None
    # زيادة نطاق البحث إلى 10 رسائل لضمان إيجاد الزر إن وجد مسبقاً
    async for m in cli.iter_messages(BOT_MRG, limit=10):
...
async def get_mrg_data_and_disconnect():
    """الاتصال مؤقتاً لجلب بيانات MRG مع تفادي إرسال start بشكل عشوائي."""
    ...
    try:
        ...
        # 1. محاولة جلب البيانات من الرسائل السابقة بدون إرسال start
        init_data = await get_init_mrg(cli, bot)
        
        # 2. في حال لم يتم العثور على الأزرار، نرسل start وننتظر رد البوت
        if not init_data:
            try: 
                await cli.send_message(BOT_MRG, "/start")
                # انتظار كافٍ (8 ثوانٍ) للسماح للبوت بالرد والتفاعل
                await asyncio.sleep(8)
            except: pass
            
            # المحاولة مرة أخرى بعد رد البوت
            init_data = await get_init_mrg(cli, bot)
...
async def mrg_main_worker():
    ...
    if not init: 
        print(f"{R}❌ جلسة MRG غير صالحة أو فشل جلب البيانات من تيليجرام{X}")
        return
        ...
        while True:
            ...
            try:
                ...
                if auth_fail or res is None:
                    ...
                    me, init = await get_mrg_data_and_disconnect()
                    if not init: 
                        print(f"⏳ [MRG] فشل استخراج البيانات، سيتم المحاولة بعد 60 ثانية (تجنباً للإغراق)...")
                        await asyncio.sleep(60) # تأخير لمنع تكرار الإرسال السريع عند الفشل
                    continue
...
async def account_worker_atf(acc_config):
    ...
    while True:
        ...
        try:
            ...
            if not init_data:
                print(f"🛑 [{acc_name}] فشل جلب initData الأولي، إعادة المحاولة لاحقاً")
                await asyncio.sleep(60) # تأخير مضاف لمنع التكرار المزعج في حسابات ATF أيضاً
                continue
